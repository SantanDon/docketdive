          ...
          <div className="flex flex-wrap gap-4 mt-3">
            ...
            {right.conditions && (
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-red-100/50 dark:bg-red-900/20 border border-red-200/50 text-[11px] font-bold text-red-600 dark:text-red-400">
                <AlertTriangle className="h-3 w-3 opacity-70" /> {right.conditions}
              </div>
            )}
          </div>
        </div>
...
export default function DocumentSimplifier({ documentContent, onClose }: DocumentSimplifierProps) {
  ...
  return (
    ...
    >
      ...
      <div className={cn("relative p-8 rounded-3xl border overflow-hidden", theme.headerBg)}>
        <div className={cn("absolute inset-0 pointer-events-none opacity-50", theme.gradient)} />
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-foreground">Document <span className={theme.textColor}>Simplifier</span></h1>
              <p className="text-muted-foreground text-sm font-medium opacity-80 mt-1">{theme.description}</p>
            </div>
          </div>
          {onClose && (
            <Button variant="ghost" size="icon" onClick={onClose} className="rounded-full h-10 w-10 hover:bg-muted/40">
              <XCircle size={20} className="text-muted-foreground/60" />
            </Button>
          )}
        </div>
      </div>
        ...
        <div className="lg:col-span-12 xl:col-span-5 space-y-8">
          {!result ? (
            <section className="space-y-6">
              {/* Task Checklist */}
              <div className="p-6 rounded-3xl bg-muted/5 border border-border/40 space-y-4">
                <h4 className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground/60 px-1">Simplification Flow</h4>
                <div className="space-y-3">
                  {[
                    { label: "Upload or Paste Content", done: !!content },
                    { label: "Generate Intelligence", done: false },
                    { label: "Refine with AI Assistant", done: false },
                  ].map((task, i) => (
              ...
              <div className="flex items-center gap-3 px-1">
                ...
                <h3 className="text-[10px] uppercase font-bold tracking-[0.2em] text-muted-foreground/60">Input Source</h3>
              </div>
              <div className="p-6 rounded-3xl bg-muted/10 border border-border/40 space-y-6">
                <DocumentDropzone
                ...
                <div className="relative h-px bg-border/40">
                  <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 bg-background text-[9px] font-bold uppercase tracking-[0.3em] text-muted-foreground/30">Direct Entry</span>
                </div>
                ...
                <div className="space-y-4">
                  <div className="relative group">
                    <textarea
                      ...
                      className={cn(
                        "w-full h-80 p-5 rounded-2xl bg-background/50 border border-border/40 transition-all duration-300",
                        "focus:ring-2 focus:ring-primary/20 focus:border-primary/40 outline-none resize-none text-sm leading-relaxed"
                      )}
                      disabled={loading}
                    />
                    {content && (
                      <div className="absolute bottom-4 right-5 text-[9px] font-bold text-muted-foreground/30 uppercase tracking-widest">
                  ...
                  {uploadedFileName && (
                    <div className="px-4 py-2 rounded-xl bg-green-500/5 border border-green-500/10 text-[10px] font-bold text-green-600/80 flex items-center gap-2">
                  ...
                  {error && (
                    <div className="p-4 bg-destructive/5 text-destructive/80 rounded-xl flex items-center gap-3 border border-destructive/10 text-[11px] font-bold">
                      <AlertCircle size={14} />
                      {error}
                    </div>
                  )}
                  ...
                  <Button
                    ...
                    className={cn(
                      ...
                      "hover:translate-y-[-1px] hover:shadow-primary/20 active:translate-y-[1px] disabled:opacity-50"
                    )}
                  >
                    {loading ? (
                      ...
                      <div className="flex items-center gap-3">
                        <Sparkles size={20} />
                        <span className="uppercase tracking-widest text-xs">Simplify Document</span>
                      </div>
                    )}
                  </Button>
                </div>
            ...
            <div className="space-y-4 sticky top-24">
              <Button
                ...
                className={cn(
                  "w-full h-14 rounded-2xl border-primary/20 hover:bg-primary/5 font-bold uppercase tracking-widest text-[10px]",
              ...
              <Button
                ...
                className="w-full h-14 rounded-2xl border-border/40 hover:bg-muted/50 font-bold uppercase tracking-widest text-[10px]"
              >
          ...
          <AnimatePresence mode="wait">
            {!result ? (
              <motion.div
                ...
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="h-full min-h-[600px] flex flex-col items-center justify-center text-center p-12 rounded-3xl bg-muted/5 border border-dashed border-border/40"
              >
                <div className="p-8 rounded-full bg-muted/10 border border-border/20 mb-6">
                  <Sparkles size={48} className="text-muted-foreground/20" />
                </div>
                <h3 className="text-2xl font-bold text-foreground mb-3 tracking-tight">Awaiting Document</h3>
                <p className="text-muted-foreground text-sm font-medium max-w-sm opacity-60">Upload or paste your legal text to see the simplified intelligence.</p>
              </motion.div>
            ) : (
              <motion.div
                key="results"
              ...
              >
                ...
                {/* Content Controls */}
                <div className="flex p-1.5 gap-1 rounded-2xl bg-muted/30 border border-border/40 items-center overflow-x-auto no-scrollbar">
                  {tabs.map((tab) => {
                    const Icon = tab.icon;
                    ...
                    return (
                      <motion.button
                        ...
                        className={cn(
                          "relative flex items-center gap-2.5 px-5 py-2.5 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all duration-300 whitespace-nowrap",
                ...
                >
                  {activeTab === "summary" && (
                    <section className="p-8 rounded-3xl bg-muted/10 border border-border/40 space-y-4">
                      <div className="flex items-center gap-3 opacity-40">
                        <FileText size={16} className="text-primary" />
                        <h3 className="text-[9px] font-bold uppercase tracking-[0.2em]">Global Overview</h3>
                      </div>
                  ...
                  {activeTab === "clauses" && (
                     <div className="space-y-3">
                       <div className="flex items-center gap-3 px-1 mb-2">
                        <div className="h-1 w-8 bg-primary rounded-full" />
                        <h3 className="text-[10px] uppercase font-bold tracking-[0.2em] text-muted-foreground/60">Clause Breakdown</h3>
                      </div>
                      <div className="space-y-3">
                    ...
                    <div className="space-y-10">
                      <div className="space-y-4">
                        <div className="flex items-center gap-2 px-1">
                          <Scale size={16} className="text-amber-500 opacity-60" />
                          <h3 className="text-[10px] uppercase font-bold tracking-[0.2em] text-muted-foreground/60">Mandatory Obligations</h3>
                        </div>
                        <div className="space-y-3">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2 px-1">
                          ...
                          <h3 className="text-[10px] uppercase font-bold tracking-[0.2em] text-muted-foreground/60">Reserved Rights</h3>
                        </div>
                        <div className="space-y-3">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-center gap-2 px-1">
                          <Clock size={16} className="text-destructive opacity-60" />
                          <h3 className="text-[10px] uppercase font-bold tracking-[0.2em] text-muted-foreground/60">Critical Deadlines</h3>
                        </div>
                        <div className="space-y-3">
